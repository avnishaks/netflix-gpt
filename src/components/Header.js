  ...
  return (
    <div className="absolute w-screen px-0 py-0 bg-gradient-to-b from-black z-10 flex justify-between p-2">
      ...
      {user && <div className='flex'>
        ...
        <button className='px-4 mx-4 my-2 bg-purple-800 text-white rounded-lg h-14' onClick={handleGPTSearchClick}>
          {showGptSearch ? "Homepage" : "GPT Search"}
        </button>
        <img className="w-12 h-12 p-2" src={user?.photoURL} alt="user_icon" />
        <button onClick={handleSingOut} className='font-bold text-white p-4 -mt-10 -px-2'>Sing Out</button>
      </div>}
    </div>
  );
}
